...
export default function AchievementsScreen({ data, onNext }: Props) {
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
    ...
    >
      <div className="absolute inset-0 bg-gradient-to-b from-yellow-500/15 via-background to-background" />
      ...
      <div className="relative z-10 flex flex-col flex-1 justify-center gap-8">
        ...
        >
          You earned some awesome achievements that are proof of your progress!
        </motion.p>
        ...
        >
          <p className="text-5xl font-black text-accent">
            {data.seasonAchievements.length}
          </p>
          <p className="text-base text-white/50 mt-1">achievements earned</p>
        </motion.div>
        ...
        >
          {data.seasonAchievements.map((ach, i) => (
            <motion.div
              ...
              className="flex items-center gap-3 p-4 bg-surface-light/50 rounded-xl border border-white/5"
            >
